...
| `common/contextkeys.ts` | `AuxiliaryBarMaximizedContext` | Context Key que define se a barra secundária está no modo maximizado, controlando a visibilidade de outras partes. |
| `visibleViewContainersTracker.ts` | `VisibleViewContainersTracker` | Rastreia a visibilidade de containers na `AuxiliaryBar` para gerenciar a auto-ocultação. |
