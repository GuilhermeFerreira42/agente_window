...
- **`WorkbenchThemeService`** $\rightarrow$ Provê a paleta de cores e gerencia a reatividade do tema via padrão `Themable`.

## 5. Resolução de Órfãos (Traceability Gaps)

| Comportamento | Arquivo | Linha | Responsabilidade |
| :--- | :--- | :--- | :--- |
| **Welcome Screen** | `vscode-main/src/vs/workbench/contrib/chat/browser/viewsWelcome/chatViewWelcomeController.ts` | N/A | Orquestra a renderização do conteúdo de boas-vindas baseado no agente e permissões. |
| **Welcome Screen (Trigger)** | `vscode-main/src/vs/workbench/contrib/chat/browser/widget/chatWidget.ts` | 1536, 1673 | `renderWelcomeViewContentIfNeeded` e `getWelcomeViewContent` determinam quando e o que exibir. |
| **Sending/Loading UI** | `vscode-main/src/vs/workbench/contrib/chat/browser/widget/chatWidget.ts` | 447, 1484 | `_isLoading` e `setTranscriptProgress` gerenciam os indicadores de progresso e estado de "enviando" na UI. |
| **Locked State (Coding Agent)** | `vscode-main/src/vs/workbench/contrib/chat/browser/widget/chatWidget.ts` | 2953, 2982 | `lockToCodingAgent` e `unlockFromCodingAgent` gerenciam a transição para o estado travado do agente de codificação. |
| **Locked State (Context)** | `vscode-main/src/vs/workbench/contrib/chat/browser/widget/chatWidget.ts` | 620, 621 | `_lockedToCodingAgentContextKey` e `_lockedCodingAgentIdContextKey` expõem o estado de trava para o sistema de contexto do VS Code. |
