...
- **Locked**: Quando o Editor (`05`) sinaliza um estado de mutação via `EditorLockService.isLocked`, a interface de chat entra em modo de leitura, desabilitando a entrada de texto para evitar conflitos de concorrência durante a aplicação de alterações por agentes.
