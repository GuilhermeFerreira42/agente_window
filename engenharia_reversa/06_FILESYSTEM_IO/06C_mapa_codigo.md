...
## 6. Resolução de Órfãos (Traceability Gaps)

| Comportamento | Arquivo | Responsabilidade |
| :--- | :--- | :--- |
| **Escritas Atômicas** | `vscode-main\src\vs\platform\files\common\files.ts` | Define `IFileAtomicWriteOptions` e a capability `FileAtomicWrite`. |
| **Prevenção de Dirty Write** | `vscode-main\src\vs\platform\files\common\fileService.ts:455-538` | Valida o `etag` e `mtime` antes de proceder com a escrita para evitar sobrescrever mudanças externas. |
| **Controle de Concorrência** | `vscode-main\src\vs\platform\files\node\diskFileSystemProvider.ts:169` | Utiliza `ResourceMap<Barrier>` para travar arquivos durante operações críticas. |
| **Sequenciamento de I/O** | `vscode-main\src\vs\platform\files\common\fileService.ts:1265` | Utiliza `ResourceQueue` para garantir a ordem sequencial de operações no mesmo recurso. |
