...
## ⚠️ AVISO CRÍTICO DE ARQUITETURA — DESACOPLAMENTO DEFINITIVO DAS CASAS (2026-09-17)
> **ATENÇÃO PARA TODAS AS IAs EXECUTORAS E DESENVOLVEDORES:**
> A arquitetura do projeto **NÃO É MAIS UNIFICADA**. O projeto foi **DESACOPLADO DE NOVO** em dois sistemas 100% independentes:
> 
> 1. **CASA VELHA (`legacy/` — Porta 5173):**
>    - Ambiente legado totalmente autônomo.
>    - **PROIBIDO IMPORTAR QUALQUER ARQUIVO DE `platform/` OU DA CASA NOVA PARA DENTRO DE `legacy/`.**
>    - Possui sua própria cópia de serviços e utilitários (ex: `legacy/services/pty-server/src/singlePort.ts`).
>    - Sobe e funciona sozinha com `npm run dev` na porta 5173.
> 
> 2. **CASA NOVA (`platform/` / `workbench-v2/` — Porta 5174):**
>    - Nova fundação modular do Agente Window com arquitetura Single Port (HTTP Upgrade via Vite).
>    - Sobe separada na porta 5174.
> 
> Não tente unificar os pacotes, diretórios ou imports de `legacy/` e `platform/`. Elas operam desacopladas.

---

## 🚀 ARQUITETURA CANÔNICA DE PORTA ÚNICA (SINGLE PORT) POR CASA (2026-09-17)
> **RESOLUÇÃO DEFINITIVA ANTIGRAVITY:**
> Cada casa do projeto opera de forma 100% autônoma em **PORTA ÚNICA** (Single Port), sem a necessidade de rodar processos separados ou abrir portas secundárias para o terminal:
>
> 1. **CASA VELHA (`legacy/`):**
>    - Roda tudo (Frontend + WebSocket do Terminal PTY) exclusivamente na porta **5173**.
>    - Possui seu próprio `legacy/singlePort.ts` e `legacy/vite-plugin-pty.ts`.
>    - Comando único: `npm run dev` (sobe apenas o Vite com HTTP Upgrade).
>    - Nenhum processo na porta 7681 é necessário.
>
> 2. **CASA NOVA (`platform/` / `workbench-v2/`):**
>    - Roda tudo (Frontend + WebSocket do Terminal PTY) exclusivamente na porta **5174**.
>    - Possui seu próprio `platform/apps/workbench-v2/singlePort.ts` e `vite-plugin-pty.ts`.
>    - Comando único: `npm run dev` na porta 5174.
>    - Terminal abre instantaneamente sem tela cinza e conecta direto no WebSocket do próprio Vite (`/pty`).
