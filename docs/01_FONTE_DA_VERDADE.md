...
## 3. Estado da Arquitetura: Desacoplamento de Casas (2026-09-17)
- **Desacoplamento Canônico:** A suposição de monorepo unificado onde o `legacy` depende do `platform` foi revogada.
- **Isolamento de Código:** `legacy/` não depende em tempo de build, execução ou tipagem de nenhum arquivo presente em `platform/`. O `vite.config.ts` da legacy não possui aliases para `platform/packages/*`, e seu plugin `vite-plugin-pty.ts` consome seu próprio `singlePort.ts` local.
- **Preservação:** O terminal do `legacy` (referência visual e funcional) e o do `platform/apps/workbench-v2/` operam de forma isolada em suas respectivas portas (5173 e 5174).
