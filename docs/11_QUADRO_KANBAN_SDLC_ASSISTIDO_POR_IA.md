...
| 2026-09-11 | 2026-09-24 | FATIA-01–03 concluídas. FATIA-04: 4.1–4.3 concluídas; **4.4 PARCIAL** (10/11 PASS medidos no preview 5174; HEAD `0d86656`); hotfix 4.8-B3 aplicado; 4.5 a fazer. Servidores: VS Code 8080, workbench 5174. | Agente (loop fechado) | main (GitHub `GuilhermeFerreira42/agente_window`) + commits locais `60287b9`…`0d86656` |
...
**Onda:** 4 | **Épico:** D — Explorer (+ C/F/E conforme sub-fatia) | **Prioridade:** P0 | **Status:** Em execução (4.1–4.3 concluídas; **4.4 PARCIAL**; 4.5 a fazer) | **Depende de:** FATIA-01 e FATIA-02 | **Doc:** `docs/engenharia_reversa/FATIA-04_VIDEO_COMPLETO/` (19 arquivos, `04_00` a `04_18`)
...
| 4.5 | **Menu completo** — geometria do host (item 24 px, separadores, coluna keybinding), teclado ↑↓/Esc devolvendo foco, Escape fecha menu (shell), **menu de contexto nos headers das seções** (`MenuId.ViewTitleContext` c/ checkmarks — exige `contextMenu.open` com `checked`), **badges numéricos na Activity Bar**, Add/Remove Folder (single-root: decidir) | `core/menus/explorerMenus.ts`, `ui/ExplorerView.tsx`, shell `ContextMenuHost` (com autorização) | `deps.menus` + `deps.contextMenu` (evolução p/ `checked`) | VAL-EXP-08 + matriz `04_03 §2` + checklist visual `04_18 §5.1` + E2E parte 2 | **A fazer** |
| 4.6 | **Search Panel** + área do anexo (sash 6 px, recolher sem desmontar, largura persistida; inputbox 26 px, toggles, Replace, include/exclude, resultados 22 px) | `ui/AttachArea.tsx`, `ui/SearchPanel.tsx`, `core/search/**`, `server/fs/searchEngine.ts` | `IEditorAttachApi`, `ISearchApi` | A5.2/A5.3/A5.7, A6.x + `04_18 §5.2` + `sessao_14_search` | **Planejado** (`04_18`) |
| 4.7 | **Editor anexo**: tabs 35 px, breadcrumbs 22 px, toolbar, empty state; **fio editor→módulo** (Open Editors real, Outline, Timeline); **cores de status Git** (bloqueado por `IDecorationsService`/GitService inexistente — criar serviço aqui) | `ui/EditorTabs.tsx`, `ui/Breadcrumbs.tsx`, `ui/CodeEditorPane.tsx`, `core/editor/editorService.ts` | `IEditorAttachApi` + dep `editors` + dep `decorations` | A5.1–A5.7, A6.4, VAL-EXP-11/12/13/14 + `04_18 §5.3` + `sessao_13_editor_anexo` | **Planejado** — 4.7b Word Wrap (Alt+Z) / Split Editor, 4.7c Markdown Preview: **deferidos** |
| 4.8 | Browser contextual (coexistir com sessão no anexo, isolamento por sessão) + IA com acesso ao HTML | — | `BrowserPort` | A7.x | **Planejado** (Q8) — **B3 hotfix já aplicado** (`876b83d`: Browser não abre no boot) |
...
**Ordem obrigatória:** 4.1 → 4.2 → 4.3 → **4.4 (fechar: T-11 + homologação manual)** → **4.5 → 4.6 → 4.7** → 4.9 (4.8 planejado; cada uma só começa com a anterior validada). **Débitos e fontes por item:** `docs/12` entrada 2026-09-24 §B; épico "Refinamento UX Explorer" em `docs/05`.
...
| 04 | Explorer Completo + Editor em anexo + Browser com IA (9 sub-fatias) | Em execução — 4.1–4.3 concluídas; **4.4 PARCIAL** (2026-09-24: 10/11 PASS na auditoria do preview 5174; T-11 pendente; G5 adiado) | Fechar 4.4 (T-11 + homologação manual) → 4.5. Medidas em `04_17`, plano em `04_18`, aceite em `04_13`, estado real em `docs/12` 2026-09-24 |
