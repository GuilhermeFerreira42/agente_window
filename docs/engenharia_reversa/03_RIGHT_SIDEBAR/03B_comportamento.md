...
A barra possui um mecanismo de otimização de espaço baseado na quantidade de conteúdo:
- **Condição**: Se `workbench.activityBar.autoHide` estiver ativo e a posição da Activity Bar for `TOP` ou `BOTTOM`.
