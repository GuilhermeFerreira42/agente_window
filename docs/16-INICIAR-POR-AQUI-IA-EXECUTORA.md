...
4. `docs/12_DOCUMENTACAO_VIVA.md` — seção 2026-09-20 LEGO (obrigatória para entender princípio de transplante)
   - **Estado vigente (2026-09-26, pós-4.6):** ler primeiro o topo de `docs/12-DOCUMENTACAO-VIVA.md` + entrada "2026-09-26 — Sub-Fatia 4.6 IMPLEMENTADA" — 4.1–4.5 **CONCLUÍDAS e homologadas**; **4.6 Search Panel + Replace IMPLEMENTADA** (7 commits `5b29da8`…`985160f`, placar `04_19 §4` 12/14; falta só o checklist humano no Windows); Search vive em `src/modules/explorer-search/{core/search,server/fs/searchEngine.ts,ui/search}` e entra no shell pelo slot `searchSlot` (App.tsx/EditorArea.tsx, exceção autorizada); **4.7 só após homologação da 4.6**; menu de contexto vive em `src/components/ExplorerContextMenuHost.tsx` (exceção de shell autorizada) + `core/menus/{explorerMenus,viewTitleMenus}.ts`; débitos no Épico D2 de `docs/05`; kanban em `docs/11`.
