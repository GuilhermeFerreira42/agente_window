...
import {
  assertBrowserViewInvariants,
  assertEditorTabInvariants,
  assertSessionInvariants,
  assertWorkbenchInvariants,
  canTransitionStatus,
  getChat,
  getDiffResolution,
  isBrowserOwnedBySession,
  isEditorTabOwnedBySession,
  isEditorTabVisibleForSession,
  navigateBrowser,
  reloadBrowser,
  resolveActiveChatId,
  setAllDiffAccepted,
  ...
  stepBrowserHistory,
  toggleDiffViewed,
  transitionChatStatus,
  transitionStatus,
  updateChatStatus,
  updateSessionAndChatStatus,
} from '../domain/sessionState'
import type { BrowserViewState, EditorTab, Session } from '../types'

const browser: BrowserViewState = {
  id: 'browser-test',
  sessionId: 's1',
  title: 'Browser',
...
const secondBrowser: BrowserViewState = {
  ...browser,
  id: 'browser-test-s2',
  sessionId: 's2',
...
const secondBrowserTab: EditorTab = {
  ...browserTab,
...
describe('session state machine and ownership', () => {
  ...
  it('removes malformed cross-links when a session owns the stale tab', () => {
    const malformedTab: EditorTab = { ...secondBrowserTab, sessionId: 's1' }
