...
import { createLayoutSync, type LayoutSyncOptions } from '../domain/sessionLayoutSync'

function makeHarness(initial: string | undefined = 's1', options?: LayoutSyncOptions) {
  const activeSessionResource = observableValue<string | undefined>('active', initial)
  const multipleSessionsVisible = observableValue('multi', false)
  const visibleSessionResources = observableValue<readonly string[]>('visible', initial ? [initial] : [])
  const captureOutgoing = vi.fn()
  const restoreIncoming = vi.fn()
  const clearVisibleSessionState = vi.fn()
  const sync = createLayoutSync(
    { activeSessionResource, multipleSessionsVisible, visibleSessionResources },
    { captureOutgoing, restoreIncoming, clearVisibleSessionState },
    options,
  )
