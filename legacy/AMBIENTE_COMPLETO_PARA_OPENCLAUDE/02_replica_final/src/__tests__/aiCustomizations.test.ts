...
const local: HarnessDescriptor = {
  id: 'local',
  label: 'Local',
  visibleSections: ['agents', 'skills', 'mcp'],
}

const copilot: HarnessDescriptor = {
  id: 'copilot',
  label: 'Copilot',
...
describe('aiCustomizations — harness filtering', () => {
  ...
  it('respeita hiddenSources', () => {
    const noPlugins: HarnessDescriptor = { ...copilot, hiddenSources: ['plugin'] }
...
describe('aiCustomizations — management model', () => {
  ...
  it('hideEmptySections omite seções sem itens', () => {
    const emptyMcp: HarnessDescriptor = { id: 'x', label: 'X', visibleSections: ['agents', 'hooks'] }
