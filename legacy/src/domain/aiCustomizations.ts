...
export const SECTION_ORDER: readonly CustomizationSection[] = [
  'agents',
  'skills',
  'instructions',
  'prompts',
...
export const SECTION_LABELS: Record<CustomizationSection, string> = {
  agents: 'Agents',
  skills: 'Skills',
...
export const SOURCE_LABELS: Record<CustomizationSource, string> = {
  local: 'Local',
  user: 'User',
  extension: 'Extension',
  plugin: 'Plugin',
...
export function applyHarnessFilter(
  ...
  return items.filter((item) => {
    if (!visible.has(item.section)) return false
    if (hidden.has(item.section)) return false
...
const SOURCE_ORDER: readonly CustomizationSource[] = ['local', 'user', 'extension', 'plugin', 'built-in']
 ...
 */
export function buildManagementModel(
  items: readonly CustomizationItem[],
  harness: HarnessDescriptor,
  enablement: EnablementState = EMPTY_ENABLEMENT,
  ...
  for (const section of SECTION_ORDER) {
    if (!harness.visibleSections.includes(section)) continue
    const sectionItems = projected.filter((item) => item.section === section)
    if (options.hideEmptySections && sectionItems.length === 0) continue
...
export function toggleEnablement(
  enablement: EnablementState,
  item: Pick<CustomizationItem, 'id' | 'source'>,
): EnablementState {
  if (item.source !== 'built-in') return enablement
  const disabled = new Set(enablement.disabled)
  if (disabled.has(item.id)) disabled.delete(item.id)
  else disabled.add(item.id)
