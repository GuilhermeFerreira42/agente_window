...
import { toggleEnablement } from './domain/aiCustomizations';
...
import { closeCustomView, customViewTitle, dismissCustomViewOnBack, dismissCustomViewOnSessionOpen, loadCustomViewState, openCustomView, restoredPartVisibility, saveCustomViewState, shouldShowCustomViewGrid, } from './domain/customView';
import { assertWorkbenchInvariants, navigateBrowser as navigateBrowserState, reloadBrowser as reloadBrowserState, resolveActiveChatId, setAllDiffAccepted, setBrowserStatus as setBrowserStatusState, setBrowserViewport as setBrowserViewportState, setDiffAccepted, stepBrowserHistory, toggleDiffViewed, updateSessionAndChatStatus, updateSessionStatus, } from './domain/sessionState';
import { getBrowserViewsForSession, getEditorTabsVisibleForSession, removeBrowserResourceForTab, removeBrowserResourcesForSession, } from './domain/browserOwnership';
...
import { isFileSystemAccessSupported, pickDirectory as pickDirectoryReal, readDirectoryEntries, readFileContent, saveRootHandle, loadRootHandle, clearRootHandle } from './domain/fileSystem';
import { captureSessionLayout, forgetSessionLayout, restoreSessionLayout } from './domain/sessionLayout';
...
import { enterDetailOnly, showEditorRestore, EMPTY_DOCKED_STATE } from './domain/dockedAuxiliaryController';
import { classifyViewport, isPhoneViewport } from './domain/mobileLayout';
import { EMPTY_NAVIGATION_STACK, navigateBack, dismissLayer, pushLayer, resetForSessionSwitch, topLayer, } from './domain/mobileNavigationStack';
...
import { readNewSessionViewState, writeNewSessionViewState, seedCreatedFromNewSession, } from './domain/newSessionViewState';
...
function isSinglePaneWidth(width) {
    if (!detectTouch())
        return false;
...
function sameComposerHistoryEntry(left, right) {
...
export default function App() {
    ...
    const [visibleSessionIds, setVisibleSessionIds] = useState([firstSession.id]);
    const [activeChatBySession, setActiveChatBySession] = useState(() => Object.fromEntries(initialSessions.map((session) => [session.id, session.mainChatId])));
    ...
    const [partSizesBySession, setPartSizesBySession] = useState(persistedLayout.current.partSizesBySession);
    const [auxiliaryTab, setAuxiliaryTab] = useState('changes');
    ...
    const sessionLayouts = useRef(loadSessionLayouts());
    // FileSystem real - integração com File System Access API (02_FILESYSTEM_WORKSPACE_REAL)
    const [fileSystemEntries, setFileSystemEntries] = useState([]);
    const [, setFileSystemRootHandle] = useState(null);
    const [fileSystemRootName, setFileSystemRootName] = useState('');
    ...
    const handleOpenFileHandle = async (entry) => {
    ...
    const activeSessionResourceObs = useRef(observableValue('activeSessionResource', firstSession.id));
    const multipleSessionsVisibleObs = useRef(observableValue('multipleSessionsVisible', false));
    const visibleSessionResourcesObs = useRef(observableValue('visibleSessionResources', [firstSession.id]));
    const liveLayoutRef = useRef({ auxiliaryVisible: persistedLayout.current.shell.auxiliaryVisible, activeViewContainerId: 'changes' });
    ...
    const [customizationEnablement, setCustomizationEnablement] = useState({ disabled: [] });
    ...
    const [checksExpandedBySession, setChecksExpandedBySession] = useState({});
    const [expandedFoldersBySession, setExpandedFoldersBySession] = useState({});
    const [editorTabs, setEditorTabs] = useState(initialEditorTabs);
    const [activeTabId, setActiveTabId] = useState(initialEditorTabs[0].id);
    const [browserViews, setBrowserViews] = useState([initialBrowser]);
    ...
    const [diffFilesBySession, setDiffFilesBySession] = useState(initialDiffFilesBySession);
    const [selectedDiffFileBySession, setSelectedDiffFileBySession] = useState({});
    ...
    const [modelByChat, setModelByChat] = useState({});
    const [modeByChat, setModeByChat] = useState({});
    const [composerDrafts, setComposerDrafts] = useState({});
    const [composerHistoryByChat, setComposerHistoryByChat] = useState({});
    ...
    const [navStack, setNavStack] = useState(EMPTY_NAVIGATION_STACK);
    ...
    const dockedController = useRef(EMPTY_DOCKED_STATE);
    // Views de browser capturadas junto com suas abas ao entrar em Detail-only.
    const capturedBrowserViews = useRef([]);
    ...
    const [newSessionState, setNewSessionState] = useState(() => readNewSessionViewState());
    // (R-087) Tema claro/escuro: lido do localStorage uma vez, alternado pelo Titlebar.
    const [theme, setTheme] = useState(() => {
        if (typeof window === 'undefined')
            return 'dark';
        return window.localStorage.getItem('agents-theme') ?? 'dark';
    });
    useEffect(() => {
    ...
    useEffect(() => {
        const next = { ...newSessionState, auxiliaryVisible };
    ...
    const pendingResponseTimers = useRef(new Map());
    const approvalTimer = useRef(undefined);
    ...
    const browserOrdinalBySession = useRef({ [firstSession.id]: 2 });
    ...
    const activeTabBySession = useRef({ [firstSession.id]: initialEditorTabs[0].id });
    const activeGlobalTabId = useRef();
    ...
    const sessionOrdinals = useRef(new Map());
    ...
    const layoutSyncRef = useRef(null);
    useEffect(() => {
        const handle = createLayoutSync({
            ...
            clearVisibleSessionState: (resources) => {
                ...
                for (const resource of resources)
                    map = forgetSessionLayout(map, resource);
    ...
    useEffect(() => () => {
        for (const timer of pendingResponseTimers.current.values())
            window.clearTimeout(timer);
        pendingResponseTimers.current.clear();
        window.clearTimeout(approvalTimer.current);
    }, []);
    useEffect(() => {
        if (skipInvariantCheck.current)
            return;
    ...
    const preferredEditorTabForSession = (allTabs, sessionId, requestedId) => {
    ...
    const openMobileLayer = (pane, layer) => {
        setMobilePane(pane);
        if (isPhone)
            setNavStack((current) => pushLayer(current, layer));
    };
    ...
    const closeVisibleSession = (id) => {
        if (!visibleSessionIds.includes(id) || visibleSessionIds.length <= 1)
            return;
    ...
    const markNestedChatRead = (sessionId, chatId) => {
    ...
    const selectEditorTab = (id) => {
        const tab = editorTabs.find((candidate) => candidate.id === id);
        if (!tab || (tab.sessionId && tab.sessionId !== activeSession.id))
            return;
    ...
    const handleReorderSessions = (fromId, toId) => {
        setSessions((current) => reorderSessions(current, fromId, toId));
    };
    const handleReorderEditorTabs = (fromId, toId) => {
        setEditorTabs((current) => reorderEditorTabs(current, fromId, toId));
    };
    const completeChatRun = (session, chatId) => {
    ...
    const clearPendingTimersForSession = (sessionId) => {
    ...
    const createBrowser = () => {
        ...
        const view = { id, sessionId, title: `Browser ${nextOrdinal}`, url, history: [url], historyIndex: 0, status: 'loading', viewport: 'desktop', reloadToken: 0 };
        const tab = { id: `${id}-tab`, type: 'browser', title: view.title, sessionId, browserId: id };
    ...
    const handlePanelLayout = (sizes) => {
        ...
        setPartSizesBySession((current) => {
            ...
            if (existing && existing.length === sizes.length && existing.every((value, index) => Math.abs(value - sizes[index]) < 0.5))
                return current;
    ...
    const openEditorTab = useCallback((type, options = { title: type }) => {
        ...
        const tab = { id: `${type}-${Date.now()}-${editorTabSequence.current++}`, type, title: options.title, path: options.path, sessionId: ownerSessionId, content: options.content, isRealFile: options.isRealFile ?? !!options.content };
        setEditorTabs((current) => [...current, tab]);
        activateEditorTab(tab);
    ...
    const openDiff = (sessionId = activeSession.id, diffFileId) => {
    ...
    const openCustomViewGrid = useCallback((view) => {
        setCustomView((current) => openCustomView(current, view, {
            sessionsPart: true,
            editor: !editorHidden,
    ...
    const openCustomizations = useCallback(() => {
        ...
        if (isPhone) {
            setNavStack((current) => pushLayer(current, { id: 'custom-view-aiCustomizations', kind: 'custom-view', label: 'AI Customizations' }));
        }
    ...
    const handleRunSkill = (item) => {
        notify(`Skill ${item.name} executada (mock)`);
    };
    const handleToggleCustomization = (item) => {
    ...
    const handleRevealCustomization = (item) => {
        notify(`Aberto: ${item.uri}`);
    };
    const customizationsSurfaceEmbedded = (_jsx(CustomizationsView, { embedded: true, items: allCustomizationItems, harnesses: harnesses, harnessId: harnessId, enablement: customizationEnablement, onChangeHarness: setHarnessId, onToggleEnablement: handleToggleCustomization, onRunSkill: handleRunSkill, onRevealItem: handleRevealCustomization }));
    const customizationsSurface = (_jsx(CustomizationsView, { items: allCustomizationItems, harnesses: harnesses, harnessId: harnessId, enablement: customizationEnablement, onChangeHarness: setHarnessId, onToggleEnablement: handleToggleCustomization, onRunSkill: handleRunSkill, onRevealItem: handleRevealCustomization }));
        ...
        setEditorHidden((current) => {
            ...
            if (next) {
                ...
                setEditorTabs((tabs) => {
                    ...
                    const capturedBrowserIds = new Set(result.state.captured
                        .map((entry) => entry.tab)
    ...
    const closeEditorTab = (id) => {
        ...
        if (!isTabCloseable(closing, sidePaneState))
            return;
        ...
        if (closing.sessionId && activeTabBySession.current[closing.sessionId] === id) {
            ...
            if (replacement)
                activeTabBySession.current[closing.sessionId] = replacement;
            else
                delete activeTabBySession.current[closing.sessionId];
        }
        if (!closing.sessionId && activeGlobalTabId.current === id)
            delete activeGlobalTabId.current;
    ...
    const navigateBrowser = (id, url) => setBrowserViews((current) => current.map((view) => view.id === id ? navigateBrowserState(view, url) : view));
    const browserBack = (id) => setBrowserViews((current) => current.map((view) => view.id === id ? stepBrowserHistory(view, -1) : view));
    const browserForward = (id) => setBrowserViews((current) => current.map((view) => view.id === id ? stepBrowserHistory(view, 1) : view));
    const reloadBrowser = (id) => setBrowserViews((current) => current.map((view) => view.id === id ? reloadBrowserState(view) : view));
    const setBrowserStatus = (id, status) => setBrowserViews((current) => current.map((view) => view.id === id ? setBrowserStatusState(view, status) : view));
    const setBrowserViewport = (id, viewport) => setBrowserViews((current) => current.map((view) => view.id === id ? setBrowserViewportState(view, viewport) : view));
    ...
    const handleAssignGroup = (id, groupId) => {
    ...
    const handleApprove = (sessionId, chatId) => {
        ...
        const timer = window.setTimeout(() => {
            if (pendingResponseTimers.current.get(runKey) !== timer)
                return;
    ...
    const handleSend = (text, attachments, target) => {
        ...
        updateSession(sessionId, (session) => {
            ...
            return {
                ...
                chats: next.chats.map((chat) => {
                    ...
                    if (chat.status !== 'working' && !hasRunningResponse)
                        return chat;
                    return {
                        ...chat,
                        status: 'completed',
    ...
    const handleRegenerate = (chatId, messageId) => {
        const sessionId = activeSession.id;
    ...
    const handleMessageCopy = (_chatId, _messageId, kind) => {
    ...
    const handleCopyFinalResponse = (chatId, messageId) => {
        const chat = activeSession.chats.find((candidate) => candidate.id === chatId);
    ...
    const handleFeedback = (chatId, messageId, vote) => {
    ...
    const handleReport = (chatId, messageId) => {
        updateSession(activeSession.id, (session) => ({
            ...session,
            updated: 'agora',
            chats: session.chats.map((chat) => chat.id === chatId ? {
    ...
    const updateDiffFilesForSession = (sessionId, updater) => {
        setDiffFilesBySession((current) => ({
            ...current,
            [sessionId]: updater(current[sessionId] ?? []),
        }));
    };
    const selectDiffFile = (id) => {
        setSelectedDiffFileBySession((current) => ({ ...current, [activeSession.id]: id }));
    };
    const acceptDiff = (id) => { updateDiffFilesForSession(activeSession.id, (current) => setDiffAccepted(current, id, true)); notify('Alteração aceita'); };
    const revertDiff = (id) => { updateDiffFilesForSession(activeSession.id, (current) => setDiffAccepted(current, id, false)); notify('Alteração revertida'); };
    ...
    const renderChatFor = (session) => {
        const sessionChatId = resolveActiveChatId(session, activeChatBySession);
        const sessionChatKey = `${session.id}:${sessionChatId}`;
        return (_jsx(ChatPanel, { session: session, activeChatId: sessionChatId, model: modelByChat[sessionChatKey] ?? selectedModel, mode: modeByChat[sessionChatKey] ?? selectedMode, auxiliaryVisible: auxiliaryVisible, onSelectChat: (chatId) => { setActiveChatBySession((current) => ({ ...current, [session.id]: chatId })); markNestedChatRead(session.id, chatId); }, onChangeModel: (nextModel) => setModelByChat((current) => ({ ...current, [sessionChatKey]: nextModel })), onChangeMode: (nextMode) => setModeByChat((current) => ({ ...current, [sessionChatKey]: nextMode })), onSend: handleSend, onStop: handleStop, onApprove: (chatId) => handleApprove(session.id, chatId), composerDrafts: composerDrafts, onChangeComposerDraft: (key, updater) => setComposerDrafts((current) => ({ ...current, [key]: updater(current[key] ?? { text: '', attachments: [] }) })), composerHistory: composerHistoryByChat, onAppendComposerHistory: (key, entry) => setComposerHistoryByChat((current) => {
                const history = current[key] ?? [];
                if (sameComposerHistoryEntry(history[history.length - 1], entry))
                    return current;
            ...
            }), onCopy: handleMessageCopy, onCopyAll: handleCopyAll, onCopyFinalResponse: handleCopyFinalResponse, onRegenerate: handleRegenerate, onFeedback: handleFeedback, onReport: handleReport, onOpenBrowser: openBrowser, onOpenDiff: () => openDiff(session.id), onToggleAuxiliary: () => setAuxiliaryVisible((current) => !current) }));
    };
    const renderChat = () => (!hasSessions ? (
    ...
    _jsx(SessionLanding, { workspace: activeSession.workspace, onSubmit: handleLandingSubmit, onPickWorkspace: handlePickDirectory, isFileSystemSupported: fileSystemSupported, onChangeMode: () => notify('Seletor de modo (mock)'), onChangeModel: () => notify('Seletor de modelo (mock)'), onAddContext: () => notify('Adicionar contexto (mock)'), onDictate: () => notify('Ditar por voz (mock)') })) : (
    ...
    const gridSessions = useMemo(() => visibleSessionIds.map((id) => sessions.find((session) => session.id === id)).filter((s) => Boolean(s)), [visibleSessionIds, sessions]);
    const renderSessionsPartGrid = () => (_jsx("div", { className: "sessions-part-grid", role: "group", "aria-label": "Sess\u00F5es vis\u00EDveis", children: gridSessions.map((session) => (_jsxs("div", { className: `sessions-part-leaf${session.id === activeSession.id ? ' is-active' : ''}`, role: "group", "aria-label": `Sessão ${session.title}`, "aria-current": session.id === activeSession.id ? 'true' : undefined, onMouseDown: () => { if (session.id !== activeSessionId)
                selectSession(session.id); }, children: [_jsxs("div", { className: "sessions-part-leaf-header", children: [_jsx("span", { className: "sessions-part-leaf-title", children: session.title }), _jsx("button", { className: "sessions-part-leaf-close", type: "button", "aria-label": `Fechar sessão ${session.title} do grid`, onMouseDown: (event) => event.stopPropagation(), onClick: (event) => { event.stopPropagation(); closeVisibleSession(session.id); }, children: _jsx(X, { size: 13 }) })] }), _jsx("div", { className: "sessions-part-leaf-body", children: renderChatFor(session) })] }, session.id))) }));
    const renderEditor = () => (_jsx(EditorArea, { activeSessionId: activeSession.id, tabs: editorTabs, activeTabId: activeTabId, browserViews: browserViews, searchQuery: searchQuery, searchResults: filteredSearchResults, searchFocusRequest: searchFocusRequest, diffFiles: activeDiffFiles, selectedDiffFileId: selectedDiffFileId, onSelectTab: selectEditorTab, onCloseTab: closeEditorTab, onReorderTabs: handleReorderEditorTabs, onNewBrowser: openBrowser, onNewFile: () => openEditorTab('file', { title: 'Files', path: 'workspace' }), onNewSearch: openSearch, onNewDiff: () => openDiff(activeSession.id), onNewCustomizations: openCustomizations, customizationsSurface: customizationsSurface, onNavigateBrowser: navigateBrowser, onBrowserBack: browserBack, onBrowserForward: browserForward, onReloadBrowser: reloadBrowser, onBrowserStatus: setBrowserStatus, onChangeViewport: setBrowserViewport, onChangeSearchQuery: setSearchQuery, onOpenSearchResult: (result) => openEditorTab('file', { title: result.path.split('/').pop() ?? result.path, path: result.path }), onSelectDiffFile: selectDiffFile, onAcceptDiff: acceptDiff, onRevertDiff: revertDiff, onAcceptAllDiff: acceptAllDiff, onRevertAllDiff: revertAllDiff, onToggleViewed: toggleViewed, onCommit: () => notify('Commit preparado para a sessão'), onCreatePr: () => notify('Pull request preparado'), editorMaximized: editorMaximized, onToggleMaximize: () => setEditorMaximized((current) => !current), onSplit: handleSplitEditor, editorContentVisible: editorContentVisible, sidePaneState: sidePaneState, onToggleEditorHidden: handleToggleEditorHidden, onToggleDetails: handleToggleDetails, detailsVisible: auxiliaryVisible, theme: theme }));
    ...
    return (_jsxs("div", { className: `app-frame agent-sessions-workbench${isSinglePane ? ' single-pane' : ''}${customViewActive ? ' custom-view-active' : ''}`, onTouchStart: handleTouchStart, onTouchEnd: handleTouchEnd, children: [_jsx(Titlebar, { activeSession: activeSession, unreadCount: unreadCount, sidebarVisible: sidebarVisible, auxiliaryVisible: auxiliaryVisible, terminalVisible: terminalVisible, approved: approved, theme: theme, onToggleTheme: handleToggleTheme, onToggleSidebar: handleToggleSidebar, onToggleAuxiliary: () => setAuxiliaryVisible((current) => !current), onToggleTerminal: () => setTerminalVisible((current) => !current), onOpenSearch: openSearch, onOpenBrowser: openBrowser, onOpenDiff: () => openDiff(activeSession.id), onNewSession: handleNewSession, onShowSessions: showSessionPicker, onAccountAction: notify }), _jsxs("div", { className: "workbench-body", style: { ['--sidebar-width']: `${sidebarWidth}px` }, children: [_jsx(SessionSidebar, { sessions: sessions, visible: sidebarVisible, activeSessionId: activeSession.id, activeChatId: activeChatId, onSelectSession: selectSession, onSelectChat: (sessionId, chatId) => { selectSession(sessionId); setActiveChatBySession((current) => ({ ...current, [sessionId]: chatId })); markNestedChatRead(sessionId, chatId); }, onNewSession: handleNewSession, onTogglePinned: (id) => updateSession(id, (session) => ({ ...session, pinned: !session.pinned })), onToggleArchived: handleToggleArchived, onDelete: handleDelete, onRename: (id, title) => updateSession(id, (session) => ({ ...session, title })), onApprove: handleApprove, onOpenDiff: openDiff, onReorderSessions: handleReorderSessions, onAssignGroup: handleAssignGroup, onRemoveGroup: handleRemoveGroup, onOpenBeside: isSinglePane ? undefined : openSessionBeside }), sidebarVisible && (_jsx("div", { className: "sidebar-resize-handle", role: "separator", "aria-orientation": "vertical", "aria-label": "Redimensionar barra de sess\u00F5es", title: "Arraste para redimensionar (duplo-clique para restaurar)", onPointerDown: startSidebarResize, onDoubleClick: resetSidebarWidth })), _jsx("div", { className: "main-region", children: _jsxs("div", { className: "right-section", children: [_jsxs("div", { className: "top-right-section", children: [_jsxs("div", { className: "main-surface", ref: surfaceGroupRef, children: [_jsxs("div", { className: "mobile-dock-tabs", role: "tablist", "aria-label": "Navega\u00E7\u00E3o single-pane", children: [isPhone && topLayer(navStack) && (_jsxs("button", { className: "dock-tab dock-back", type: "button", "aria-label": "Voltar navega\u00E7\u00E3o", title: "Voltar", onClick: () => handleMobileBack(), children: [_jsx(ChevronLeft, { size: 13 }), "Voltar"] })), _jsxs("button", { className: `dock-tab${mobilePane === 'chat' ? ' is-active' : ''}`, type: "button", onClick: () => { setMobilePane('chat'); setNavStack(EMPTY_NAVIGATION_STACK); }, children: [_jsx(CheckCircle2, { size: 13 }), "Chat"] }), _jsxs("button", { className: `dock-tab${mobilePane === 'editor' ? ' is-active' : ''}`, type: "button", onClick: () => openMobileLayer('editor', { id: 'editor', kind: 'full-screen-editor', label: 'Editor' }), children: [_jsx(Info, { size: 13 }), "Editor", visibleEditorTabs.length > 0 ? ` · ${visibleEditorTabs.length}` : ''] }), _jsxs("button", { className: `dock-tab${mobilePane === 'details' ? ' is-active' : ''}`, type: "button", onClick: () => openMobileLayer('details', { id: 'details', kind: 'drawer', label: 'Detalhes' }), children: [_jsx(Info, { size: 13 }), "Detalhes"] })] }), customViewActive && customView.activeView ? (
                                                ...
                                                _jsxs("section", { className: "custom-view-grid", role: "region", "aria-label": `Custom View: ${customViewTitle(customView.activeView)}`, "data-custom-view": customView.activeView, children: [_jsxs("header", { className: "custom-view-header", children: [_jsx("h1", { className: "custom-view-title", children: customViewTitle(customView.activeView) }), _jsxs("button", { className: "secondary-button", type: "button", "aria-label": "Fechar custom view", onClick: closeCustomViewGrid, children: [_jsx(X, { size: 13 }), "Fechar"] })] }), _jsx("div", { className: "custom-view-content", children: customizationsSurfaceEmbedded })] })) : (_jsx(_Fragment, { children: isSinglePane ? (_jsxs("div", { className: "desktop-surface-group", children: [mobilePane === 'chat' && renderChat(), mobilePane === 'editor' && renderEditor(), mobilePane === 'details' && (
                                                    ...
                                                    editorContentVisible ? (editorMaximized ? (_jsx("div", { className: `desktop-surface-group editor-maximized side-pane-${sidePaneState}`, children: renderEditor() })) : (_jsxs(PanelGroup, { ref: panelGroupRef, direction: "horizontal", className: `desktop-surface-group side-pane-${sidePaneState}`, onLayout: handlePanelLayout, "data-flexible-part": "sessions", children: [_jsx(Panel, { defaultSize: activePartSizes[0], minSize: 34, order: 1, children: renderChat() }), _jsx(PanelResizeHandle, { className: "panel-resize-handle", onDoubleClick: resetPartSizes, title: "Duplo-clique para redistribuir igualmente" }), _jsx(Panel, { defaultSize: activePartSizes[1], minSize: 28, order: 2, children: renderEditor() })] }, activeSession.id))) : tabBarVisible ? (
                                                    // Detail-only / editor oculto: mantém a barra de abas, mas o chat
                                                    // absorve a largura do conteúdo do editor.
                                                    _jsxs("div", { className: `desktop-surface-group side-pane-${sidePaneState}`, children: [_jsx("div", { className: "chat-region", children: renderChat() }), renderEditor()] })) : (
                                                    ...
                                                    _jsx("div", { className: `desktop-surface-group side-pane-${sidePaneState}${chatCentered ? ' chat-centered' : ''}`, children: renderChat() }))) }))] }), !customViewActive && layoutController.managesAuxiliaryBar && _jsx(AuxiliaryBar, { session: activeSession, visible: renderDesktopAuxiliaryBar, fileSystemEntries: fileSystemEntries, fileSystemRootName: fileSystemRootName, fileSystemLoading: fileSystemLoading, isFileSystemSupported: fileSystemSupported, onPickDirectory: handlePickDirectory, onClearDirectory: handleClearDirectory, onOpenFileHandle: handleOpenFileHandle, diffFiles: activeDiffFiles, tab: auxiliaryTab, checksExpanded: checksExpanded, expandedFolders: expandedFolders, onChangeTab: setAuxiliaryTab, onOpenDiff: (fileId) => openDiff(activeSession.id, fileId), onOpenFile: (path) => openEditorTab('file', { title: path.split('/').pop() ?? path, path }), onToggleChecks: toggleChecks, onToggleFolder: toggleFolder, onRerunChecks: rerunChecks, onOpenCheck: openCheck, onPreparePr: preparePullRequest, onMerge: mergeChanges, onOpenTerminal: openTerminalFromChanges, onClose: () => setAuxiliaryVisible(false) })] }), _jsx(TerminalPanel, { visible: terminalVisible && !customViewActive, sessionId: activeSession.id, sessionLabel: activeSessionLabel, workspace: activeSession.workspace, onClose: () => setTerminalVisible(false) })] }) })] }), toast && _jsxs("div", { className: "toast", role: "status", children: [_jsx(Info, { size: 14 }), _jsx("span", { children: toast }), _jsx("button", { type: "button", "aria-label": "Fechar aviso", title: "Fechar aviso", onClick: () => setToast(undefined), children: _jsx(X, { size: 13 }) })] }), _jsx(SessionsPicker, { open: sessionPickerOpen, sessions: sessions, activeSessionId: activeSession.id, onClose: () => setSessionPickerOpen(false), onSelectSession: (id) => { selectSession(id); setMobilePane('chat'); }, onNewSession: handleNewSession })] }));
}
