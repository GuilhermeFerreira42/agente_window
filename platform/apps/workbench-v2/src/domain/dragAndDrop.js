 ...
 */
export function canReorderSessions(from, to) {
    if (!from || !to || from.id === to.id)
        return false;
    if (from.archived || to.archived)
        return false;
    if (from.section === 'archived' || to.section === 'archived')
        return false;
...
export function reorderEditorTabs(tabs, fromId, toId) {
    ...
    if (!from || !to || from.sessionId !== to.sessionId)
        return tabs;
