...
export function registerDiscovery(registry, registration) {
 ...
 */
export function aggregatePlugins(discoveries, options = {}) {
    if (options.pluginsEnabled === false)
        return [];
    if (!isServiceReady(discoveries))
        return [];
...
export function isPluginEnabled(plugin, enablement = EMPTY_PLUGIN_ENABLEMENT) {
    return !enablement.disabled.includes(plugin.canonicalId);
}
/** Alterna o enablement de um plugin por identidade canônica (imutável). */
export function togglePluginEnablement(enablement, canonicalId) {
    const disabled = new Set(enablement.disabled);
    if (disabled.has(canonicalId))
        disabled.delete(canonicalId);
    else
        disabled.add(canonicalId);
 ...
 * contribuem itens.
 */
export function projectPluginContributions(plugins, enablement = EMPTY_PLUGIN_ENABLEMENT) {
    const items = [];
    for (const plugin of plugins) {
        if (!isPluginEnabled(plugin, enablement))
            continue;
