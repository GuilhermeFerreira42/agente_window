...
export function ExplorerView({ service, menus, contextMenu, fs, baseUrl }: ExplorerViewProps): React.ReactElement {
  ...
  // ---- context keys (04_03 §7 / 04_10 §2.4) — publicadas via menus.setContext
  // em cada selectionChanged/operação; derivação pura em core/menus/explorerMenus.
    ...
    return computeExplorerContext({
      ...
      target: item
        ? {
          ...
          parentReadonly: item.parent?.isReadonly ?? false,
        }
        : null,
  ...
  const publishContext = React.useCallback((contextUri?: WorkspaceUri | null) => {
    ...
    for (const key of Object.keys(ctx) as Array<keyof ExplorerContextValues>) {
      menus.setContext(key, ctx[key]);
    }
    return ctx;
  ...
  React.useEffect(() => {
    ...
    return service.onEvent((e) => {
      if (e.type === 'explorer.selectionChanged' || e.type === 'explorer.rootChanged' || e.type === 'fs.changed') {
        publishContext();
      }
    });
  }, [service, publishContext]);

  // Menu de contexto (04_03 §1): right-click na linha → tabela declarativa
  // resolvida por `when`/`precondition` contra as context keys; o host do shell
  // só desenha e chama menus.execute(id) (VAL-EXP-08 — zero lógica inline).
  const openItemContextMenu = React.useCallback((input: { uri: WorkspaceUri | null; x: number; y: number }) => {
    const ctx = publishContext(input.uri);
    contextMenu.open({ x: input.x, y: input.y, items: resolveExplorerContextMenu(ctx) });
  }, [contextMenu, publishContext]);

  // Área vazia da árvore → contexto da raiz (04_18 §4.5 item 5).
  const onTreeContextMenu = React.useCallback((ev: React.MouseEvent) => {
    ...
    if (!root) return;
    service.select({ uris: [root.resource] });
    openItemContextMenu({ uri: root.resource, x: ev.clientX, y: ev.clientY });
  }, [service, openItemContextMenu]);
  ...
  // Upload a partir de arquivos do SO (drop OU picker) — mesmo pipeline
  // core/transfer/upload (octet-stream atômico, mkdir pais, conflito, progresso).
  ...
  }, [menus, beginCreate, refreshAll, collapseAll, downloadSelection, uploadViaPicker, service, beginRename, tick]);
  ...
  const onItemDrop = React.useCallback((ev: React.DragEvent) => {
    ...
    void collectDroppedFiles(ev.dataTransfer.items).then(
      (records) => {
        dragData.current = null;
        if (records.length === 0) { setTransferStatus(null); return; }
        runUpload(plan.target.resource, records);
      },
      (err: Error) => {
        setTransferStatus({ kind: 'upload', text: `Upload falhou: ${err.message}` });
        setTimeout(() => setTransferStatus(null), 4000);
      },
    );
  }, [buildData, dndCtx, service, tick, runUpload]);
  ...
  return (
    <div
      ...
      onDragLeave={() => setDropTargetUri(null)}
    ...
    >
      {/* Upload... (04_03 §1 5b_importexport/20): picker do SO → mesmo pipeline do drop */}
      <input
        ref={uploadInputRef}
        type="file"
        multiple
