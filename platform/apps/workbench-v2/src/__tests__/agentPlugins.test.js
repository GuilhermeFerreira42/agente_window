...
import { EMPTY_PLUGIN_ENABLEMENT, aggregatePlugins, createDiscoveryRegistry, isPluginEnabled, isServiceReady, projectPluginContributions, registerDiscovery, togglePluginEnablement, } from '../domain/agentPlugins';
...
describe('agent plugins — prontidão e agregação (R-065)', () => {
    ...
    it('o master switch pluginsEnabled=false esconde todos os plugins', () => {
        const discoveries = [{ id: 'a', priority: 1, plugins: [plugin()] }];
...
describe('agent plugins — enablement e projeção (R-065)', () => {
    ...
    it('plugins desabilitados não contribuem itens', () => {
        const plugins = [plugin({ canonicalId: 'acme/off' })];
...
//# sourceMappingURL=agentPlugins.test.js.map