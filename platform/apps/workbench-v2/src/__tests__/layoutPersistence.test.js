...
import { DEFAULT_SHELL, LAYOUT_STORAGE_KEY, SIDEBAR_WIDTH_DEFAULT, SIDEBAR_WIDTH_MAX, SIDEBAR_WIDTH_MIN, clampSidebarWidth, defaultLayoutState, loadLayoutState, partSizesForSession, saveLayoutState, } from '../domain/layoutPersistence';
...
describe('layoutPersistence', () => {
    ...
    it('degrades gracefully when no storage is available', () => {
        expect(loadLayoutState(undefined)).toEqual(defaultLayoutState());
        // Não deve lançar.
        expect(() => saveLayoutState(defaultLayoutState(), undefined)).not.toThrow();
    });
});
//# sourceMappingURL=layoutPersistence.test.js.map