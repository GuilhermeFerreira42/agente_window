...
describe('usePtySession', () => {
    ...
    it('connects to the integrated /pty websocket without port discovery', async () => {
        const { result } = renderHook(() => usePtySession({ sessionId: 'session-test-1' }));
    ...
    it('handles output messages and sends input', async () => {
        const { result } = renderHook(() => usePtySession({ sessionId: 'session-test-2' }));
        await vi.waitFor(() => {
            expect(MockWebSocket.instances.length).toBe(1);
        });
        const ws = MockWebSocket.instances[0];
        ...
        expect(ws.send).toHaveBeenCalledWith(JSON.stringify({ type: 'input', sessionId: 'session-test-2', data: 'dir\r' }));
    });
    it('clears the buffered output for future listeners without touching the socket', async () => {
        const { result } = renderHook(() => usePtySession({ sessionId: 'session-test-clear' }));
        await vi.waitFor(() => {
            expect(MockWebSocket.instances.length).toBe(1);
        });
        const ws = MockWebSocket.instances[0];
    ...
    it('handles websocket failures and transitions to error state', async () => {
        const { result } = renderHook(() => usePtySession({ sessionId: 'session-test-err' }));
        await vi.waitFor(() => {
            expect(MockWebSocket.instances.length).toBe(1);
        });
        act(() => {
...
//# sourceMappingURL=usePtySession.test.js.map