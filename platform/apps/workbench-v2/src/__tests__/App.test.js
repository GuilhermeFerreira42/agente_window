...
function sessionRow(title) {
    const titleElement = Array.from(document.querySelectorAll('.session-row .session-title-text'))
        .find((element) => element.textContent === title);
    const row = titleElement?.closest('.session-row');
    if (!row)
        throw new Error(`Session row not found: ${title}`);
    return row;
}
function browserEditorTab() {
    return document.querySelector('.editor-tab[role="tab"]');
}
// O botão "Novo Browser" foi removido; um novo browser agora é criado pelo
// menu do "+" do editor (Adicionar aba do editor → Browser).
async function createBrowserTab(user) {
...
function nestedChat(title) {
    const element = Array.from(document.querySelectorAll('.nested-chat-title'))
        .find((candidate) => candidate.textContent === title);
    if (!element)
        throw new Error(`Nested chat not found: ${title}`);
    return element;
}
describe('App session flows', () => {
    ...
    it('conecta regenerar e feedback ao estado persistido da mensagem', async () => {
        ...
        const responseRow = responseText.closest('.chat-message');
        if (!responseRow)
            throw new Error('Assistant message row not found');
        ...
        const codeResponseRow = screen.getByText(/Entendido\. Vou manter/).closest('.chat-message');
        if (!codeResponseRow)
            throw new Error('Code response row not found');
    ...
    it('preserva o anexo do rascunho ao alternar a superfície mobile', async () => {
        ...
        try {
            ...
            if (originalTouch)
                Object.defineProperty(window.navigator, 'maxTouchPoints', originalTouch);
    ...
    it('preserva seleção de chat e oculta/mostra Browser conforme ownership da sessão', async () => {
        const user = userEvent.setup();
        render(_jsx(App, {}));
        const nestedUiChat = Array.from(document.querySelectorAll('.nested-chat-title'))
            .find((element) => element.textContent === 'Ajustes de UI');
        if (!nestedUiChat)
            throw new Error('Nested UI chat not found');
    ...
    it('cria, seleciona, navega, recarrega, reporta erro e fecha múltiplas abas Browser no Editor', async () => {
        const user = userEvent.setup();
        render(_jsx(App, {}));
        const editorTabs = () => Array.from(document.querySelectorAll('.editor-tab[role="tab"]'));
        const browserFrame = () => screen.getByTitle(/Browser https:\/\//);
        ...
        if (!secondTab)
            throw new Error('Second Browser tab not found');
        ...
        const closeTab = async (title) => {
            ...
            if (!tab)
                throw new Error(`Browser tab not found: ${title}`);
            const close = tab.querySelector('.editor-tab-close');
            if (!close)
                throw new Error(`Close button not found: ${title}`);
    ...
    it('seleciona a próxima aba correta ao fechar e expõe o tabpanel ativo', async () => {
        ...
        render(_jsx(App, {}));
        const editorTabs = () => Array.from(document.querySelectorAll('.editor-tab[role="tab"]'));
        const tabByTitle = (title) => editorTabs().find((tab) => tab.querySelector('.editor-tab-title')?.textContent === title);
        const close = async (title) => {
            const tab = tabByTitle(title);
            if (!tab)
                throw new Error(`Editor tab not found: ${title}`);
        ...
        if (!browserTwo)
            throw new Error('Browser 2 tab not found');
    ...
    it('navega pelas tabs por teclado e cria Files, Search e Changes no menu de nova aba', async () => {
        ...
        render(_jsx(App, {}));
        await createBrowserTab(user);
        const browserTwo = Array.from(document.querySelectorAll('.editor-tab[role="tab"]')).find((tab) => tab.querySelector('.editor-tab-title')?.textContent === 'Browser 2');
        if (!browserTwo)
            throw new Error('Browser 2 tab not found');
        browserTwo.focus();
    ...
    it('preserva Browser por sessão e remove todos os browsers ao arquivar ou excluir', async () => {
        ...
        if (!s1SecondTab)
            throw new Error('Second session Browser tab not found');
    ...
    it('abre Search no Editor com foco, filtra, conta, destaca, mostra vazio e abre arquivo', async () => {
        ...
        const resultButton = document.querySelector('.search-result');
        if (!resultButton)
        ...
        await user.click(screen.getByRole('button', { name: 'Abrir alterações no editor' }));
        expect(screen.getByRole('tab', { name: /Branch Changes/ })).toHaveAttribute('aria-selected', 'true');
        const browserTab = document.querySelector('.editor-tab[role="tab"][data-browser-id]');
        if (!browserTab)
            throw new Error('Browser editor tab not found after opening Search result');
    ...
    it('abre Branch Changes como aba do Editor, lista arquivos, seleciona o diff e preserva a seleção por sessão', async () => {
        ...
        const browserTab = document.querySelector('.editor-tab[role="tab"][data-browser-id]');
        if (!browserTab)
            throw new Error('Browser editor tab not found');
        await user.click(browserTab);
        await user.click(screen.getByRole('tab', { name: /Branch Changes/ }));
        expect(document.querySelector('.diff-selected-editor[data-selected-diff-id="diff-3"]')).toBeInTheDocument();
        await user.click(sessionRow('Revisar alterações do workbench'));
    ...
    it('executa as ações de Diff por arquivo e em lote com feedback observável e isolamento por sessão', async () => {
        ...
        await user.click(screen.getByRole('button', { name: 'Abrir alterações no editor' }));
        const entry = (id) => {
            const element = document.querySelector(`.diff-entry[data-diff-file-id="${id}"]`);
            if (!element)
                throw new Error(`Diff entry not found: ${id}`);
            return element;
        };
    ...
    it('mostra empty state coerente quando a sessão não possui alterações', async () => {
        const user = userEvent.setup();
        render(_jsx(App, {}));
        const newSessionButton = document.querySelector('.titlebar [aria-label="Nova sessão"]');
        if (!newSessionButton)
    ...
    it('navega entre sessões com as setas do teclado', () => {
        render(_jsx(App, {}));
        const rows = Array.from(document.querySelectorAll('.session-row[data-session-nav="true"]'));
    ...
    it('abre e fecha uma sessão peer no Sessions Part grid', async () => {
        ...
        const grid = document.querySelector('.sessions-part-grid');
    ...
    it('a Changes pill do cabeçalho abre o diff e revela o editor (E10/R-083)', async () => {
        ...
        const headerPill = document.querySelector('.chat-pane-header .changes-pill');
        if (!headerPill)
            throw new Error('Changes pill do cabeçalho não encontrada');
    ...
    it('persiste a visibilidade do terminal entre reloads (remontagem)', async () => {
        const user = userEvent.setup();
        window.localStorage.clear();
        const first = render(_jsx(App, {}));
        // Liga o terminal (parte oculto por padrão) e desmonta (simula fechar a aba).
        const terminalToggle = screen.getByRole('button', { name: 'Alternar terminal' });
        if (!terminalToggle.classList.contains('is-active'))
            await user.click(terminalToggle);
    ...
    it('persiste os tamanhos do split por sessão no storage', async () => {
        ...
        const anySizes = Object.values(parsed.partSizesBySession)[0];
    ...
    it('restaura a largura da barra de sessões com duplo-clique no sash', async () => {
        ...
        window.localStorage.setItem('workbench.sessions.layout.v1', JSON.stringify({ shell: { sidebarVisible: true, auxiliaryVisible: true, terminalVisible: false, editorHidden: false, sidebarWidth: 360 }, partSizesBySession: {} }));
        render(_jsx(App, {}));
    ...
    it('protege abas gerenciadas (Files) de fechamento em detail-only', async () => {
        ...
        const filesTab = Array.from(document.querySelectorAll('.editor-tab[role="tab"]'))
            .find((tab) => tab.querySelector('.editor-tab-title')?.textContent === 'Files');
        expect(filesTab).toBeTruthy();
        expect(within(filesTab).queryByRole('button', { name: 'Fechar Files' })).not.toBeInTheDocument();
        expect(filesTab.querySelector('.editor-tab-managed')).not.toBeNull();
    ...
    it('abre menu de contexto num chat aninhado da lista de sessões', async () => {
        render(_jsx(App, {}));
        const chat = nestedChat('Ajustes de UI').closest('.nested-chat-row');
        if (!chat)
            throw new Error('Nested chat row not found');
    ...
    it('reordena sessões da mesma seção por drag & drop', async () => {
        ...
        const order = () => Array.from(document.querySelectorAll('.session-row .session-title-text')).map((el) => el.textContent);
        const before = order();
    ...
    it('anexa ao chat um arquivo arrastado da árvore do workspace', async () => {
        ...
        const fileRow = document.querySelector('.file-tree-file');
        if (!fileRow)
            throw new Error('File tree row not found');
    ...
    it('reordena abas do editor da mesma sessão por drag & drop', async () => {
        ...
        await createBrowserTab(user);
        const tabs = () => Array.from(document.querySelectorAll('.editor-tab[role="tab"]'));
