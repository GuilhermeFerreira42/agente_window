...
describe('TerminalGroup', () => {
    it('renderiza o sash no split e recalcula o ratio dentro do clamp', () => {
        ...
        const { container } = render(_jsx(TerminalGroup, { split: true, splitRatio: 0.5, onSplitRatioChange: onSplitRatioChange, mainPane: _jsx("div", { children: "Main" }), splitPane: _jsx("div", { children: "Split" }) }));
        const root = container.querySelector('.terminal-group');
    ...
    it('não renderiza sash quando não há split', () => {
        render(_jsx(TerminalGroup, { split: false, splitRatio: 0.5, onSplitRatioChange: () => { }, mainPane: _jsx("div", { children: "Main" }) }));
        expect(screen.queryByRole('separator', { name: 'Redimensionar split do terminal' })).not.toBeInTheDocument();
    });
});
//# sourceMappingURL=TerminalGroup.test.js.map