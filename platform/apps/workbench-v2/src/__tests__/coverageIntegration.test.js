...
describe('P8.2 ChatInput — handlers interativos', () => {
    it('insere menção (@) e ditado por voz usa fallback sem SpeechRecognition', async () => {
        ...
        expect(input.value).toContain('Descreva a alteração');
    ...
    it('adiciona e remove um anexo do workspace', async () => {
        ...
        if (!chosen)
            throw new Error('Nenhum arquivo de workspace disponível no picker');
...
describe('P8.2 ChatPanel — welcome e novo chat', () => {
    function renderPanel(overrides = {}) {
        ...
        return { ...render(_jsx(ChatPanel, { ...props })), props };
    }
    it('dispara sugestão do welcome e cria novo chat', async () => {
        ...
        if (!suggestion)
            throw new Error('Sugestão do welcome não encontrada');
...
describe('P8.2 AuxiliaryBar — ações de checks e diff', () => {
    ...
    function renderAux(overrides = {}) {
        ...
        return { ...render(_jsx(AuxiliaryBar, { ...props })), props };
    }
...
//# sourceMappingURL=coverageIntegration.test.js.map