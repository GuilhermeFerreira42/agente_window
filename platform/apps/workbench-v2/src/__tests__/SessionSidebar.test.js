...
function renderSidebar() {
    ...
    return { ...render(_jsx(SessionSidebar, { ...props })), props };
}
describe('SessionSidebar', () => {
    it('aplica is-hidden e aria-hidden quando não está visível', () => {
        const { props, unmount } = renderSidebar();
        unmount();
        const view = render(_jsx(SessionSidebar, { ...props, visible: false }));
    ...
    it('seleciona chat filho por teclado (Enter) e aprova aprovação do chat', async () => {
        ...
        render(_jsx(SessionSidebar, { ...props, sessions: [sessionWithApproval] }));
        await user.click(screen.getByRole('button', { name: 'Expandir chats' }));
        // Seleção do chat filho via teclado (Enter) sobre a linha role="button".
        const childRow = screen.getByText('Chat filho').closest('[role="button"]');
        childRow.focus();
    ...
    it('filtra sessões pelo campo da sidebar mantendo a sessão ativa sempre visível', async () => {
        ...
        const { props, unmount } = renderSidebar();
        unmount();
        const otherSession = { ...sessions[0], id: 's-other', title: 'Outra sessão', chats: [], mainChatId: 'chat-main' };
        const view = render(_jsx(SessionSidebar, { ...props, sessions: [sessions[0], otherSession], activeSessionId: "s-test" }));
    ...
    it('não duplica uma sessão fixada no agrupamento temporal e recolhe a seção', async () => {
        ...
        const view = render(_jsx(SessionSidebar, { ...props, sessions: [pinnedToday] }));
    ...
    it('encaminha ações, aprovação, rename por F2 e estado de atenção', async () => {
        ...
        render(_jsx(SessionSidebar, { ...props, sessions: [attentionSession] }));
    ...
    it('agrupa Quick Chats, grupos personalizados e exclui runs de automação', () => {
        ...
        render(_jsx(SessionSidebar, { ...props, sessions: many, activeSessionId: "d1" }));
    ...
    it('filtra por estado de leitura (não lidas)', async () => {
        ...
        render(_jsx(SessionSidebar, { ...props, sessions: items, activeSessionId: "x" }));
    ...
    it('aplica capping de workspaces e promove o workspace ativo', async () => {
        ...
        const items = ['alpha', 'bravo', 'charlie', 'delta', 'echo'].map((ws, index) => ({
            ...sessions[0],
            id: `s-${ws}`,
            title: `Sessão ${ws}`,
            workspace: `ws-${ws}`,
        ...
        render(_jsx(SessionSidebar, { ...props, sessions: items, activeSessionId: "s-echo" }));
...
//# sourceMappingURL=SessionSidebar.test.js.map