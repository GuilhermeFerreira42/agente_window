...
function buildSession(pid, label = 'Bash', shellId = 'bash', path = '/bin/bash') {
    return {
        status: 'open',
        pid,
...
function renderPanel(overrides = {}) {
    return render(_jsx(TerminalPanel, { ...baseProps, ...overrides }));
}
describe('TerminalPanel', () => {
    ...
    it('abre o menu de contexto do terminal e dispara selecionar tudo / encerrar processo', async () => {
        const user = userEvent.setup();
        renderPanel();
        const terminal = document.querySelector('.terminal-container.is-active');
        const instance = xtermMock.MockTerminal.instances[0];
    ...
    it('descarta a instância anterior ao trocar de sessão', () => {
        ...
        view.rerender(_jsx(TerminalPanel, { ...baseProps, sessionId: "session-two", workspace: "workspace-two" }));
...
//# sourceMappingURL=TerminalPanel.test.js.map