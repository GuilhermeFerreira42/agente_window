...
import {
  CUSTOM_VIEW_STORAGE_KEY,
  closeCustomView,
  customViewTitle,
  dismissCustomViewOnBack,
  dismissCustomViewOnSessionOpen,
  effectivePartVisibility,
  emptyCustomViewState,
  isCustomViewId,
  loadCustomViewState,
  openCustomView,
...
const userWants: PartVisibility = { sessionsPart: true, editor: false, auxiliaryBar: true, panel: true }
...
describe('customView — exclusividade e visibilidade efetiva', () => {
  ...
  it('trocar de custom view NÃO recaptura a visibilidade (evita salvar o estado coberto)', () => {
    const aberta = openCustomView(emptyCustomViewState(), 'aiCustomizations', userWants)
    const coberto: PartVisibility = { sessionsPart: false, editor: false, auxiliaryBar: false, panel: false }
